...
namespace CodingMuscles.CSharpInnoSetup.Example64BitTwoArch
{
    class Program
    {
        ...
        public override Action<IParameterizedEntriesBuilder> ParameterizedEntriesBuilderHandler => builder =>
        {
            ...
            contentBuilder.AddFiles((builder, languages, types, customMessage, messages, components, tasks) =>
            {
                return new[]
                {
                    ...
                    builder().Source(_source, "MyProg-x64.exe").Destination(__app).DestinationName("MyProg.exe").Check(fn => Is64BitInstallMode()).Build(),
                    builder().Source(_source, "MyProg.exe").Destination(__app).Check(fn => !Is64BitInstallMode()).Flags(FileFlags.SolidBreak).Build(),
